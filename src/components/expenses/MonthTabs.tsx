...
export const MonthTabs = ({ selectedMonth, onMonthChange }: MonthTabsProps) => {
  ...
  useEffect(() => {
    if (tabsRef.current) {
      const currentMonthTab = tabsRef.current.querySelector(
        `[data-value="${currentMonth}"]`,
      );
      if (currentMonthTab) {
        currentMonthTab.scrollIntoView({
          behavior: "smooth",
          block: "nearest",
          inline: "center",
        });
  ...
  return (
    ...
    >
      ...
      >
        <TabsList
          className="h-16 items-center justify-start w-full overflow-x-scroll no-scrollbar"
