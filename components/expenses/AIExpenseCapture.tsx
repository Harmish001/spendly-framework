...
export const AIExpenseCapture = ({
  ...
  useEffect(() => {
    // Listen for shared images dispatched via CustomEvent (web only)
  ...
  const processImageWithAI = async (imageFile: File) => {
    ...
    try {
      ...
      // Convert image to base64 (for future AI integration)
      ...
      const data = {
        ...
        description: "Mocked AI Extracted Receipt",
      };

      console.log("Received mocked data from AI:", data);
      ...
      toast.error(
        (error as Error).message || "Failed to process image. Please try again.",
      );
    } finally {
